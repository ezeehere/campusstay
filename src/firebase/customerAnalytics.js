...
export async function getAdminCustomerAnalytics() {
    ...
    return students.map((student) => {
        const studentSaved = savedListings.filter(
            (item) => item.studentId === student.uid || item.studentId === student.id
        );

        const studentEvents = analyticsEvents.filter(
            (event) => event.studentId === student.uid || event.studentId === student.id
        );

        const studentLeads = leads.filter(
            (lead) => lead.studentId === student.uid || lead.studentId === student.id
        );
        ...
        return {
            ...
            searchText: safeLower(
                `${student.fullName || ""} ${student.email || ""} ${student.phone || ""} ${student.college || ""} ${student.preferredArea || ""}`
            ),
        };
    });
}