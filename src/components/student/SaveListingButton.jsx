  ...
  useEffect(() => {
    const unsubscribe = watchStudentAuth(async (user) => {
      ...
      try {
        ...
        console.warn("Saved check failed:", error);
        setSaved(false);
      } finally {
        setChecking(false);
      }
  ...
  return (
    <>
      <button
        ...
        className={`inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-2 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-60 ${saved
